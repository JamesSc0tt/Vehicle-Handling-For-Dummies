local errprefix = "^3[vhfd_builders]^7 ^1ERROR:^7 "
--[[
	Translators
]]
...
function builder.shouldBuild(res)
	return GetNumResourceMetadata(res, "vhfd_handling") > 0
end
...
function builder.build(res, finished)
	...
	for i=1, GetNumResourceMetadata(res, "vhfd_handling") do
		local file = GetResourceMetadata(res, "vhfd_handling", i-1)
		xml:comment(file)
...
RegisterResourceBuildTaskFactory("vhfd_handling", factory)